<?php
if(file_exists(str_replace("\\", "/", dirname(__FILE__)).'/json.php')) include(str_replace("\\", "/", dirname(__FILE__)).'/json.php');
...
class googleTranslateTool{
    ...
    function translate_Text($text) {
		...
		$url="http://ajax.googleapis.com/ajax/services/language/translate?format=html&v=1.0";
        
        $postData=array();
        //$postData['text']= $text;
		$postData['q']= $text;
        $postData['langpair'] ="|".$this->langTo;
		...
		return $output['responseData']['translatedText'];    
    }
    ...
    function translate_URL($url) {
        ...
        $langpair = "|".$this->langTo;        
        
        $url='http://66.102.9.104/translate_c?hl=ro&safe=off&ie=UTF-8&oe=UTF-8&langpair='.$langpair.'&u='.urlencode($url);        
    ...
    function getData_Curl($url,$postData=false){
		...
		curl_setopt($curlHandle, CURLOPT_REFERER, $HTTP_REFERER);
